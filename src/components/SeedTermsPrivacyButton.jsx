...
export default function SeedTermsPrivacyButton() {
    ...
    <div className="p-6 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
          {isArabic ? '🌱 تهيئة صفحات الشروط والخصوصية' : '🌱 Initialiser Pages Termes & Confidentialité'}
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {isArabic 
            ? 'انقر لملء صفحات شروط الاستخدام وسياسة الخصوصية بمحتوى افتراضي شامل. يمكنك تحريرها لاحقاً.'
            : 'Cliquez pour remplir les pages Termes et Confidentialité avec un contenu complet par défaut. Vous pourrez les modifier ensuite.'
          }
        </p>
      </div>
      ...
      {status === 'success' && (
        <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg flex items-start gap-3">
          ...
          <div className="text-sm text-green-700 dark:text-green-400">
            {isArabic 
              ? '✅ تم تهيئة الصفحات بنجاح! يمكنك الآن تحريرها من قائمة الصفحات.'
              : '✅ Pages initialisées avec succès ! Vous pouvez maintenant les modifier depuis la liste des pages.'
            }
          </div>
        </div>
      )}
      ...
      {status === 'error' && (
        <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg flex items-start gap-3">
          ...
          <div className="text-sm text-red-700 dark:text-red-400">
            {isArabic 
              ? '❌ خطأ في التهيئة. يرجى المحاولة مرة أخرى.'
      ...
      >
        {loading ? (
          <>
            ...
            {isArabic ? 'جاري التهيئة...' : 'Initialisation en cours...'}
          </>
        ) : status === 'success' ? (
          <>
            <CheckCircleIcon className="w-5 h-5" />
            {isArabic ? 'تم التهيئة' : 'Déjà initialisé'}
          </>
        ) : (
          isArabic ? '🌱 تهيئة الصفحات' : '🌱 Initialiser les pages'
        )}
      </button>

      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {isArabic ? '📝 المحتوى المضمن:' : '📝 Contenu inclus:'}
        </h4>
        <ul className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
          <li>• {isArabic ? 'شروط الاستخدام (9 أقسام)' : 'Conditions d\'Utilisation (9 sections)'}</li>
          <li>• {isArabic ? 'سياسة الخصوصية (10 أقسام)' : 'Politique de Confidentialité (10 sections)'}</li>
          <li>• {isArabic ? 'محتوى ثنائي اللغة (فرنسية/عربية)' : 'Contenu bilingue (Français/Arabe)'}</li>
          <li>• {isArabic ? 'معلومات شاملة ومتوافقة' : 'Informations complètes et conformes'}</li>
        </ul>
      </div>
    </div>
  );
